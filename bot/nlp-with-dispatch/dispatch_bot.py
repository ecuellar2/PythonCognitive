...
class DispatchBot(ActivityHandler):
    ...
    async def _dispatch_to_top_intent(
        ...
        if intent == "FindFlights":
            ...
            await self._process_weather(turn_context, recognizer_result.properties["luisResult"])
    ...
    async def _process_home_automation(
        ...
        await turn_context.send_activity(
            f"Top intent {luis_result.top_scoring_intent}."
        )
        ...
        await turn_context.send_activity(
            f"Intents detected: {intents_list}."
        )
        ...
        if luis_result.entities:
            ...
            await turn_context.send_activity(
                f"entities were found in the message: {entities_list}."
            )
